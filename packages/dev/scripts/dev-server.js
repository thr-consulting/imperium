...
if (cluster.isMaster) {
	...
	log.notice('Imperium Master startup (Workers: 1)', {environment: util.inspect(process.env, true, null, false)});
	...
	cluster.on('exit', (deadWorker, code, signal) => {
		...
		if (workerCrashCounter < workerCrashMax) {
			...
			log.emerg('Worker thread keeps crashing, exiting main app...');
			setTimeout(() => {
				process.exit(1);
			}, 400);
		}
